...
extension String {
    ...
    func extractMillage() -> (Range<String.Index>, String)?  {
        guard let range = self.range(of: "\\d{6,7}", options: .regularExpression, range: nil, locale: nil) else {
	...
	func extractPhoneNumber() -> (Range<String.Index>, String)? {
        ...
        let regex = try! NSRegularExpression(pattern: "\\w{5,7}")
        let range = NSRange(location: 0, length: self.utf8.count)
        if let val = regex.firstMatch(in: self, options: [], range: range) {
            print(val)
        }
		...
		guard phoneNumberDigits.count == 6 else {
			return nil
		}
